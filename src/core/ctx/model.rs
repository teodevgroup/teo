...
impl ModelCtx {
    ...
    pub async fn find_unique<T: From<Object>>(&self, finder: &Value) -> Result<Option<T>> {
        AppCtx::get()?.graph().find_unique(self.model, finder, self.conn.clone(), self.req()).await
    }

    pub async fn find_first<T: From<Object>>(&self, finder: &Value) -> Result<Option<T>> {
        AppCtx::get()?.graph().find_first(self.model, finder, self.conn.clone(), self.req()).await
    }

    pub async fn find_many<T: From<Object>>(&self, finder: &Value) -> Result<Vec<T>> {
        AppCtx::get()?.graph().find_many(self.model, finder, self.conn.clone(), self.req()).await
    }

    pub async fn create_object<T: From<Object>>(&self, values: &Value) -> Result<T> {
        AppCtx::get()?.graph().create_object(self.model, values, self.conn.clone(), self.req()).await.map(|v| v.into())
    }

    pub async fn count(&self, finder: &Value) -> Result<usize> {
        AppCtx::get()?.graph().count(self.model, finder, self.conn.clone()).await
    }

    pub async fn aggregate<T: From<Value>>(&self, finder: &Value) -> Result<T> {
        AppCtx::get()?.graph().aggregate(self.model, finder, self.conn.clone()).await.map(|v| v.into())
    }

    pub async fn group_by<T: From<Value>>(&self, finder: &Value) -> Result<Vec<T>> {
        let vec_value = AppCtx::get()?.graph().group_by(self.model, finder, self.conn.clone()).await?;
