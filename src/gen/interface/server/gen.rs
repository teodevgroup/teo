...
pub(crate) async fn gen(graph: &Graph, conf: &Conf) -> Result<()> {
    match conf.provider {
        ProgramLang::Rust => generate_entity_typed(RustEntityGenerator::new(), &Ctx::build(graph, conf, RustTypes::new())).await,
        ProgramLang::Go => generate_entity_typed(GoEntityGenerator::new(), &Ctx::build(graph, conf, SwiftTypes::new())).await,
        ProgramLang::NodeJS => generate_entity_typed(NodeJSEntityGenerator::new(), &Ctx::build(graph, conf, SwiftTypes::new())).await,
        ProgramLang::Python => generate_entity_typed(PythonEntityGenerator::new(), &Ctx::build(graph, conf, SwiftTypes::new())).await,
        ProgramLang::Java => generate_entity_typed(JavaEntityGenerator::new(), &Ctx::build(graph, conf, SwiftTypes::new())).await,
