...
use teo_runtime::data_set::DataSet;
use teo_runtime::namespace::Namespace;

pub(crate) async fn seed(action: SeedCommandAction, namespace: &'static Namespace, datasets: Vec<DataSet>, ctx: transaction::Ctx) -> Result<()> {
    // seed for user
    for dataset in &datasets {
        match action {
            SeedCommandAction::Seed => seed_dataset(namespace, dataset, ctx.clone()).await,
            SeedCommandAction::Reseed => reseed_dataset(namespace, dataset, ctx.clone()).await,
            SeedCommandAction::Unseed => unseed_dataset(namespace, dataset, ctx.clone()).await,
        }
    }
...
pub(crate) async fn seed_dataset(namespace: &'static Namespace, dataset: &DataSet, ctx: transaction::Ctx) {
    let ordered_groups = ordered_group(&dataset.groups, namespace);
    ...
    for group in &ordered_groups {
        ...
        for record in group.records.iter() {
            ...
            if !existing {
                perform_insert_into_database(dataset, group, record, group_model, namespace, connection.clone()).await;
        ...
        for seed_record in seed_records.iter() {
            ...
            if !existing {
                perform_remove_from_database(dataset, seed_record, group_model, namespace, connection.clone()).await;
            }
        }
    }
    // Second, setup optional relations and array relations
    setup_new_relations(namespace, dataset, &ordered_groups, Some(&added_records), connection.clone()).await;
    // Last, remove records for user removed groups
    remove_records_for_user_removed_groups(dataset, &ordered_groups, namespace, connection.clone()).await;
}

async fn remove_records_for_user_removed_groups(dataset: &'static DataSet, ordered_groups: &Vec<&'static Group>, namespace: &'static Namespace, ctx: transaction::Ctx) {
    ...
    for record in user_removed_seed_records_for_group {
        ...
        if model.is_some() {
            perform_remove_from_database(dataset, &record, model.unwrap(), namespace, connection.clone()).await;
        } else {
...
pub(crate) async fn reseed_dataset(namespace: &'static Namespace, dataset: &'static DataSet, ctx: transaction::Ctx) {
    let ordered_groups = ordered_group(&dataset.groups, namespace);
    for group in &ordered_groups {
        let group_model = AppCtx::get().unwrap().model(group.model_path()).unwrap().unwrap();
        let seed_records = GroupRecord::find_many(teon!({
        ...
        for record in group.records.iter() {
            if let Some(seed_record) = seed_records.iter().find(|r| &r.name() == &record.name) {
                // recreate or update
                perform_recreate_or_update_an_record(dataset, group, record, group_model, namespace, seed_record, connection.clone()).await;
            } else {
                // create
                perform_insert_into_database(dataset, group, record, group_model, namespace, connection.clone()).await;
            }
    ...
    // Second, setup optional relations and array relations
    sync_relations(namespace, dataset, &ordered_groups, connection.clone()).await;
    // Last, remove records for user removed groups
    remove_records_for_user_removed_groups(dataset, &ordered_groups, namespace, connection.clone()).await;
}

pub(crate) async fn unseed_dataset(namespace: &'static Namespace, dataset: &'static DataSet, ctx: transaction::Ctx) {
    let mut ordered_groups = ordered_group(&dataset.groups, namespace);
    ...
    for group in ordered_groups {
        ...
        for seed_record in seed_records.iter() {
            ...
            perform_remove_from_database(dataset, seed_record, model, namespace, connection.clone()).await;
        }
    }
}

async fn sync_relations(namespace: &'static Namespace, dataset: &'static DataSet, ordered_groups: &Vec<&'static Group>, ctx: transaction::Ctx) {
    for group in ordered_groups {
        let group_model = AppCtx::get().unwrap().model(group.model_path()).unwrap().unwrap();
        ...
        for record in group.records.iter() {
            ...
            let object: Object = namespace.find_unique(group_model, &teon!({
                ...
                if let Some(reference) = record.value.as_hashmap().unwrap().get(relation.name()) {
                    if let Some(references) = reference.as_vec() {
                        for reference in references {
                            sync_relation_internal(record, reference, relation, dataset, namespace, &object, &relation_records, &mut relation_record_refs, connection.clone()).await;
                        }
                    } else {
                        sync_relation_internal(record, reference, relation, dataset, namespace, &object, &relation_records, &mut relation_record_refs, connection.clone()).await;
                    }
                    ...
                    for relation_record in relation_record_refs {
                        cut_relation(relation_record, seed_record, namespace, group_model, dataset, &object, connection.clone()).await;
                    }
                }
            }
        }
...
async fn setup_new_relations(namespace: &'static Namespace, dataset: &'static DataSet, ordered_groups: &Vec<&'static Group>, limit: Option<&HashMap<String, Vec<String>>>, ctx: transaction::Ctx) {
    for group in ordered_groups {
        let group_model = AppCtx::get().unwrap().model(group.model_path()).unwrap().unwrap();
        let should_process = group_model.relations().iter().find(|r| !(r.has_foreign_key() && r.is_required())).is_some();
        if !should_process { continue }
        ...
        for record in group.records.iter() {
            ...
            for relation in group_model.relations() {
                if let Some(reference) = record.value.as_hashmap().unwrap().get(relation.name()) {
                    if let Some(references) = reference.as_vec() {
                        for reference in references {
                            setup_relations_internal(record, reference, relation, dataset, namespace, &object, connection.clone()).await;
                        }
                    } else {
                        setup_relations_internal(record, reference, relation, dataset, namespace, &object, connection.clone()).await;
                    }
...
async fn sync_relation_internal<'a>(record: &'static Record, reference: &'a Value, relation: &'static Relation, dataset: &'static DataSet, namespace: &'static Namespace, object: &'a Object, relation_records: &'a Vec<GroupRelation>, relation_record_refs: &mut Vec<&'a GroupRelation>, ctx: transaction::Ctx) {
...
async fn setup_relations_internal<'a>(record: &'static Record, reference: &'a Value, relation: &'static Relation, dataset: &'static DataSet, namespace: &'static Namespace, object: &'a Object, ctx: transaction::Ctx) {
    let that_name = reference.as_raw_enum_choice().unwrap();
    ...
    let that_object: Object = namespace.find_unique(relation.model(), &teon!({
    ...
    if relation.is_optional() && relation.has_foreign_key() {
        ...
        let (through_model, through_relation) = namespace.through_relation(relation);
        let (_, through_that_relation) = namespace.through_opposite_relation(relation);
        ...
        let link_record: Option<Object> = namespace.find_first(through_model, &teon!({
            "where": Value::HashMap(where_unique.clone())
        }), connection.clone(), None).await.unwrap();
        if link_record.is_none() {
            let link_object = namespace.create_object(through_model, Value::HashMap(where_unique), connection.clone(), None).await.unwrap();
    ...
    if exist_relation_record.is_err() {
        // not exist, create
        let that_relation = namespace.opposite_relation(relation).1;
...
async fn perform_remove_from_database<'a>(dataset: &'static DataSet, record: &'a GroupRecord, group_model: &'static Model, namespace: &'static Namespace, ctx: transaction::Ctx) {
    let json_identifier = record.record();
    let exist: Option<Object> = namespace.find_unique(group_model, &teon!({
    ...
    for relation in relations {
        cut_relation(&relation, record, namespace, group_model, dataset, &exist, connection.clone()).await;
    }
...
async fn cut_relation<'a>(relation: &'a GroupRelation, record: &'a GroupRecord, namespace: &'static Namespace, group_model: &'static Model, dataset: &'static DataSet, exist: &'a Object, ctx: transaction::Ctx) {
    ...
    let that_record: Option<Object> = namespace.find_unique(that_model, &teon!({
            "where": unique
        }), connection.clone(), None).await.unwrap();
    ...
    if model_relation.has_join_table() {
        let (through_model, through_relation) = namespace.through_relation(model_relation);
        let (_, through_that_relation) = namespace.through_opposite_relation(model_relation);
        let mut where_unique: HashMap<String, Value> = HashMap::new();
        for (local, foreign) in through_relation.iter() {
...
async fn perform_recreate_or_update_an_record<'a>(dataset: &'static DataSet, group: &'static Group, record: &'static Record, group_model: &'static Model, namespace: &'static Namespace, seed_record: &'a GroupRecord, ctx: transaction::Ctx) {
    let object: Option<Object> = namespace.find_unique(group_model, &teon!({
    ...
    let input = insert_or_update_input(dataset, group, record, group_model, namespace, connection.clone()).await;
...
async fn insert_or_update_input(dataset: &'static DataSet, group: &'static Group, record: &'static Record, group_model: &'static Model, namespace: &'static Namespace, ctx: transaction::Ctx) -> Value {
    let mut input = teon!({});
    for (k, v) in record.value.as_hashmap().unwrap() {
        if group_model.field(k).is_some() {
            ...
            if relation.is_required() && relation.has_foreign_key() {
                ...
                let that_record: Object = namespace.find_unique(relation_model, &teon!({
...
async fn perform_insert_into_database(dataset: &'static DataSet, group: &'static Group, record: &'static Record, group_model: &'static Model, namespace: &'static Namespace, ctx: transaction::Ctx) {
    let input = insert_or_update_input(dataset, group, record, group_model, namespace, connection.clone()).await;
    let object = namespace.create_object(group_model, &input, connection.clone(), None).await.unwrap();
    object.save_for_seed_without_required_relation().await.unwrap();
...
fn ordered_group<'a>(groups: &'a Vec<Group>, namespace: &'static Namespace) -> Vec<&'a Group> {
