...
pub(crate) fn is_email(_args: Vec<Argument>) -> Arc<dyn Modifier> {
    Arc::new(IsEmailModifier::new())
}

pub(crate) fn is_alphabetic(_args: Vec<Argument>) -> Arc<dyn Modifier> {
    Arc::new(IsAlphabeticModifier::new())
}

pub(crate) fn is_numeric(_args: Vec<Argument>) -> Arc<dyn Modifier> {
    Arc::new(IsNumericModifier::new())
}

pub(crate) fn is_alphanumeric(_args: Vec<Argument>) -> Arc<dyn Modifier> {
    Arc::new(IsAlphanumericModifier::new())
}

pub(crate) fn is_secure_password(_args: Vec<Argument>) -> Arc<dyn Modifier> {
    Arc::new(IsSecurePasswordModifier::new())
}

pub(crate) fn regex_match(args: Vec<Argument>) -> Arc<dyn Modifier> {
    let value = args.get(0).unwrap().resolved.as_ref().unwrap().as_value().unwrap();
    Arc::new(RegexMatchModifier::new(value))
}

pub(crate) fn has_prefix(args: Vec<Argument>) -> Arc<dyn Modifier> {
    let value = args.get(0).unwrap().resolved.as_ref().unwrap().as_value().unwrap();
    Arc::new(HasPrefixModifier::new(value))
}

pub(crate) fn has_suffix(args: Vec<Argument>) -> Arc<dyn Modifier> {
    let value = args.get(0).unwrap().resolved.as_ref().unwrap().as_value().unwrap();
    Arc::new(HasSuffixModifier::new(value))
}

pub(crate) fn is_prefix_of(args: Vec<Argument>) -> Arc<dyn Modifier> {
    let value = args.get(0).unwrap().resolved.as_ref().unwrap().as_value().unwrap();
    Arc::new(IsPrefixOfModifier::new(value))
}

pub(crate) fn is_suffix_of(args: Vec<Argument>) -> Arc<dyn Modifier> {
    let value = args.get(0).unwrap().resolved.as_ref().unwrap().as_value().unwrap();
    Arc::new(IsSuffixOfModifier::new(value))
}

pub(crate) fn is_hex_color(_args: Vec<Argument>) -> Arc<dyn Modifier> {
