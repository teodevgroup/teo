    ...
    let graph = Graph::new(|g| {
        ...
        g.model("OneOptionalLocal", |m| {
            ...
            m.relation("foreign", |r| {
                r.optional().object("OneOptionalForeign").fields(vec!["foreignId"]).references(vec!["id"]);
            });
        });
        g.model("OneOptionalForeign", |m| {
            ...
            m.relation("local", |r| {
                r.optional().object("OneOptionalLocal").fields(vec!["id"]).references(vec!["foreignId"]);
            });
        });
    }).await;
...
async fn update_with_nested_connect() {
    ...
    let id = id.as_str().unwrap();
    let foreign_id = request_get(&app, "one-optional-foreigns", "Create", teon!({
        "create": {},
    }), 200, "data.id").await;
    let foreign_id = foreign_id.as_str().unwrap();
    let res = request(&app, "one-optional-locals", "Update", teon!({
