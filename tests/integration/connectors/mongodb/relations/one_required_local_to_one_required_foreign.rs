    ...
    let graph = Graph::new(|g| {
        ...
        g.model("OneRequiredLocal", |m| {
            ...
            m.relation("foreign", |r| {
                r.required().object("OneRequiredForeign").fields(vec!["foreignId"]).references(vec!["id"]);
            });
        });
...
async fn create_with_nested_create() {
    ...
    let res = request(&app, "one-required-locals", "Create", teon!({
        "create": {
            "foreign": {
                "create": {}
            }
...
async fn create_with_nested_connect() {
    let app = test::init_service(app().await).await;
    let foreign_id = request_get(&app, "one-required-locals", "Create", teon!({
        "create": {
            "foreign": {
                "create": {}
            }
...
async fn update_with_nested_create() {
    let app = test::init_service(app().await).await;
    let id = request_get(&app, "one-required-locals", "Create", teon!({
        "create": {
            "foreign": {
                "create": {}
            }
    ...
    let res = request(&app, "one-required-locals", "Update", teon!({
        "where": {
            "id": id
        },
        "update": {
...
async fn update_with_nested_disconnect() {
    let app = test::init_service(app().await).await;
    let ids = request_get(&app, "one-required-locals", "Create", teon!({
        "create": {
            "foreign": {
                "create": {}
            }
...
async fn include() {
    ...
    let id = ids.as_vec().unwrap().get(0).unwrap().as_str().unwrap();
    let foreign_id = ids.as_vec().unwrap().get(1).unwrap().as_str().unwrap();
    let res = request(&app, "one-required-locals", "FindUnique", teon!({
        "where": {"id": id},
        "include": {
            "foreign": true
        }
